...
fn get_answer(input: &str) -> Option<usize> {
    let mut line1: HashSet<(isize, isize)> = HashSet::new();
    let mut cross: HashSet<(isize, isize)> = HashSet::new();
    ...
    let mut mmd = usize::MAX;
    for c in cross {
        let md = manhattan_dist(c);
        if md < mmd {
            mmd = md;
        }
    }
...
fn manhattan_dist(c: (isize, isize)) -> usize {
    (c.0.abs() + c.1.abs()) as usize
}
